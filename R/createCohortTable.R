...
#'
#' This is a function which can create a  Cohort Table
#'
#'
#' @param connection
...
loadCohortTable_v <- function(connection,
  ...
  sql_v <- "SELECT a.cohort_definition_id, a.subject_id, a.cohort_start_date, b.visit_concept_id, d.concept_name, b.visit_start_date, b.visit_end_date, b.care_site_id, c.care_site_name
             FROM @cohort_database_schema.@cohort_table a
             ...
             left outer join @cdm_database_schema.CONCEPT d on b.visit_concept_id = d.concept_id;"
...
#' @export loadCohortTable_c
loadCohortTable_c <- function(connection,
                              cohortDatabaseSchema,
                              cohortTable
){

  sql_c <- "SELECT a.cohort_definition_id, a.subject_id, a.cohort_start_date, b.condition_concept_id, c.concept_name, b.condition_start_date, b.condition_end_date
             FROM @cohort_database_schema.@cohort_table a
  ...
  sql_c <- SqlRender::render(sql_c,
                             cohort_database_schema = cohortDatabaseSchema,
                             cdm_database_schema = cdmDatabaseSchema,
                             cohort_table = cohortTable)

  df_c <- as.data.frame(DatabaseConnector::querySql(connection, sql_c))

  return(df_c)
}

#' @export loadCohortTable_p
loadCohortTable_p <- function(connection,
                               cohortDatabaseSchema,
                               cohortTable
){

  sql_p <- "SELECT a.cohort_definition_id, a.subject_id, a.cohort_start_date, b.procedure_concept_id, c.concept_name, b.procedure_date
             FROM @cohort_database_schema.@cohort_table a
             left outer join @cdm_database_schema.procedure_occurrence b on a.subject_id = b.person_id
             left outer join @cdm_database_schema.CONCEPT c on b.procedure_concept_id = c.concept_id;"

  sql_p <- SqlRender::render(sql_p,
                              cohort_database_schema = cohortDatabaseSchema,
                              cdm_database_schema = cdmDatabaseSchema,
                              cohort_table = cohortTable)

  df_p <- as.data.frame(DatabaseConnector::querySql(connection, sql_p))
